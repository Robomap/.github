...
export async function robomapSso(
  request: FastifyRequest<{ Body: { access_token?: string; returnUrl?: string } }>,
  reply: FastifyReply
) {
  try {
    ...
    const exchangeData = (await exchangeRes.json()) as { user?: RobomapUser };
    ...
    const hasRobomapAccount = (accounts || []).some(
      (account: { providerId?: string }) => account.providerId === "robomap"
    );
