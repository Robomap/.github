...
if (workerCount === 0) {
    ...
    try {
        ...
        try {
            ...
            logger.error({ err: chError }, "ClickHouse initialization failed; continuing so auth/API can serve");
            logger.info("Postgres initialization complete (ClickHouse unavailable)");
        }
    }
    catch (error) {
    ...
    function broadcastUsageState() {
        ...
        logger.debug(`Broadcasted ${overLimitIds.length} sites-over-limit and ${withoutReplayIds.length} sites-without-replay to workers`);
    }
    // Graceful shutdown
    const shutdown = async (signal) => {
        ...
        const activeWorkers = Object.values(cluster.workers ?? {}).filter((w) => w != null && !w.isDead());
        const workerExitPromises = activeWorkers.map(worker => new Promise(resolve => {
